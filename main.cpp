...
RPS_DECLARE_RPSL_ENTRY(hello, main);
...
struct CommandPool {
  bool b_in_use = false;
  uint32_t un_pool_index = 0;
  VkCommandPool resource = VK_NULL_HANDLE;
  VkCommandBuffer command_buffer;

  void MarkUnused() { b_in_use = false; }
};
...
class Program {
 public:
  ...
  bool Init() {
    {  //Vulkan instance initialization
      ...
      {  // logical device creation
        ...
        std::set<uint32_t> set_unique_queue_families = {m_queue_family_indices.opt_graphics_family.value(),
                                                        m_queue_family_indices.opt_present_family.value()};
        ...
        vkGetDeviceQueue(m_vkdevice, m_queue_family_indices.opt_graphics_family.value(), 0, &m_vkgraphics_queue);
        vkGetDeviceQueue(m_vkdevice, m_queue_family_indices.opt_present_family.value(), 0, &m_vkpresent_queue);
      }

      {  //rps device create info
        ...
        RpsDeviceCreateInfo rps_device_create_info = {
            .allocator = {},
            .printer = {},
            .privateDataAllocInfo = {},
            .pfnDeviceOnDestroy = nullptr,
        };
        RpsVKRuntimeDeviceCreateInfo rps_vk_runtime_device_create_info = {
            .pDeviceCreateInfo = &rps_device_create_info,
        ...
        if (RpsResult res = rpsVKRuntimeDeviceCreate(&rps_vk_runtime_device_create_info, &m_rpsdevice)) {
          std::cout << "rpsVKRuntimeDeviceCreate failed with: " << res << std::endl;
          return false;
        }
      }
      ...
      {  //create rps render graph
        ...
        RpsRenderGraphCreateInfo rps_render_graph_create_info = {
            ...
            .memoryInfo = {},
            .mainEntryCreateInfo = {.hRpslEntryPoint = RPS_ENTRY_REF(hello, main)},
            .numPhases = 0,
            .pPhases = nullptr,
        };
        if (RpsResult res = rpsRenderGraphCreate(m_rpsdevice, &rps_render_graph_create_info, &m_rpsrendergraph)) {
          std::cout << "rpsRenderGraphCreate failed with: " << res << std::endl;
          return false;
        }

        if (RpsResult res = rpsProgramBindNode(rpsRenderGraphGetMainEntry(m_rpsrendergraph), "Triangle",
                                               &Program::RpsDrawTriangle, this)) {
          std::cout << "rpsProgramBindNode failed with: " << res << std::endl;
          return false;
        }
      }
  ...
  void Tick() {
    {
      RpsRuntimeResource back_buffer_resources[16];

      for (uint32_t i = 0; i < m_swapchain_images.size(); i++) {
        back_buffer_resources[i] = {m_swapchain_images[i]};
      }
      const RpsRuntimeResource* p_arg_resources[] = {back_buffer_resources};

      RpsResourceDesc back_buffer_desc = {
          .type = RPS_RESOURCE_TYPE_IMAGE_2D,
          .temporalLayers = uint32_t(m_swapchain_images.size()),
          .flags = 1,
          .image =
              {
                  ...
                  .arrayLayers = 1,
                  .mipLevels = 1,
                  .format = rpsFormatFromVK(m_swapchain_format.format),
                  .sampleCount = 1,
              },
      };
      ...
      RpsRenderGraphUpdateInfo render_graph_update_info = {
          ...
          .numArgs = 1,
          .ppArgs = arg_data,
          .ppArgResources = p_arg_resources,
          .pfnBuildCallback = nullptr,
          .pRandomNumberGenerator = nullptr,
      };
      if (RpsResult res = rpsRenderGraphUpdate(m_rpsrendergraph, &render_graph_update_info)) {
        std::cout << "rpsRenderGraphUpdate failed: " << res << std::endl;
      }
    }
    ...
    vkResetFences(m_vkdevice, 1, &mv_frame_fences[mun_backbuffer_index].vkfence_render_complete);
    ...
    {  //reserve semaphores for batches
      const uint32_t un_old_size = uint32_t(mv_queue_semaphores.size());
      if (render_graph_batch_layout.numFenceSignals > un_old_size) {
        mv_queue_semaphores.resize(render_graph_batch_layout.numFenceSignals, VK_NULL_HANDLE);
      }

      VkSemaphoreCreateInfo semaphoreCI = {VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO};

      for (size_t i = un_old_size; i < render_graph_batch_layout.numFenceSignals; i++) {
        v_qualify_vk(vkCreateSemaphore(m_vkdevice, &semaphoreCI, nullptr, &mv_queue_semaphores[i]));
      }
    }

    for (uint32_t i = 0; i < render_graph_batch_layout.numCmdBatches; i++) {
      auto& batch = render_graph_batch_layout.pCmdBatches[i];

      CommandPool* p_use_command_pool;
      {  //allocate a command buffer for this batch
        if (mv_command_pools.size() <= m_swapchain_images.size()) {
          mv_command_pools.resize(m_swapchain_images.size());
        }

        uint32_t un_command_pool_free_index = 0;
        while (un_command_pool_free_index < mv_command_pools[mun_backbuffer_index].size()) {
          if (!mv_command_pools[mun_backbuffer_index][un_command_pool_free_index].b_in_use) {
            break;
          }

          un_command_pool_free_index++;
        }

        if (un_command_pool_free_index == mv_command_pools[mun_backbuffer_index].size()) {
          VkCommandPoolCreateInfo ci{VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO};
          ci.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
          ci.queueFamilyIndex = m_queue_family_indices.opt_graphics_family.value();
        ...
        VkCommandBufferAllocateInfo command_buffer_allocate_info = {
            ...
            .pNext = nullptr,
            .commandPool = mv_command_pools[mun_backbuffer_index][un_command_pool_free_index].resource,
            .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY,
            .commandBufferCount = 1,
        };
        v_qualify_vk(
            vkAllocateCommandBuffers(m_vkdevice, &command_buffer_allocate_info, &p_use_command_pool->command_buffer));
      }

      {
        VkCommandBuffer record_command_buffer = p_use_command_pool->command_buffer;
        ...
        v_qualify_vk(vkBeginCommandBuffer(record_command_buffer, &command_buffer_begin_info));
        ...
        if (RpsResult res = rpsRenderGraphRecordCommands(m_rpsrendergraph, &record_command_info)) {
          std::cout << "rpsRenderGraphRecordCommands failed: " << res << std::endl;
        }
        vkEndCommandBuffer(record_command_buffer);

        {  //submit
          bool b_frame_end = ((i + 1) == render_graph_batch_layout.numCmdBatches);
          VkPipelineStageFlags submit_wait_stage = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;

          uint32_t un_wait_semaphores = 0;
          ...
          const uint32_t* idx = render_graph_batch_layout.pWaitFenceIndices + batch.waitFencesBegin;
          for (uint32_t j = 0; j < batch.numWaitFences && j < RPS_MAX_QUEUES; ++j) {
            wait_semaphores[un_wait_semaphores++] = mv_queue_semaphores[idx[j]];
          }

          VkFence vkfence_submit = VK_NULL_HANDLE;
          uint32_t un_count_signal_semaphores = 0;
          VkSemaphore signal_semaphores[2] = {};

          if (b_frame_end) {
            m_vksem_pending_present = mv_frame_fences[mun_backbuffer_index].vksem_render_complete;
            signal_semaphores[un_count_signal_semaphores] = m_vksem_pending_present;
            ++un_count_signal_semaphores;

            vkfence_submit = mv_frame_fences[mun_backbuffer_index].vkfence_render_complete;
          }

          if (batch.signalFenceIndex != UINT32_MAX) {
            signal_semaphores[un_count_signal_semaphores] = mv_queue_semaphores[batch.signalFenceIndex];
            ++un_count_signal_semaphores;
          }
          ...
          VkSubmitInfo submit_info = {
              ...
              .pNext = nullptr,
              .waitSemaphoreCount = un_wait_semaphores,
              .pWaitSemaphores = wait_semaphores,
              .pWaitDstStageMask = &submit_wait_stage,
              .commandBufferCount = 1,
              .pCommandBuffers = &record_command_buffer,
              .signalSemaphoreCount = un_count_signal_semaphores,
              .pSignalSemaphores = signal_semaphores,
          };
          v_qualify_vk(vkQueueSubmit(m_vkgraphics_queue, 1, &submit_info, vkfence_submit));
        }
        p_use_command_pool->MarkUnused();
      }
 ...
 private:
  ...
  std::vector<std::vector<CommandPool>> mv_command_pools;
